...
async function recordMissedAggregatorProgress(expectedRound, expectedAggregator, reason, error = null) {
    ...
    try {
        ...
        console.warn("Could not inspect the aggregation policy before timeout reporting; " +
            "deferring the report until the on-chain policy is observable:", policyError?.message || String(policyError));
        return;
    }
...
async function uploadLocalModel(endpoint, deviceId, expectedRound, expectedAggregator, parentModel) {
    ...
    const signature = crypto.sign('sha256', signingPayload, activeParticipantKey.privateKey).toString('base64');
        ...
        switch (state["0"]) {
            case "TRAINING":
                if (sameAddress(state[1], process.env.ACCOUNT_ADDRESS)) {
                    ...
                    if (openedPolicy.closed) {
                        console.log(`Round ${currentRound} input policy is already closed; resuming aggregation.`);
                        await setCurrentState("AGGREGATING");
                        continue;
                    }
                    if (currentRound === 0) {
                        console.log("Round 0 bootstrap phase: closing the empty input set and proceeding directly to aggregation.");
                    ...
                    try {
                        ...
                        console.error("Aggregator receive/close step failed; keeping TRAINING for reconciliation:", e);
                        await sleep(2000);
                        continue;
                    }
                }
                    ...
                    if ((process.env.DATASET_NAME || "mnist").toLowerCase() === "chestmnist") {
                        medicalSignerSnapshot = await runOperation("worker.fetch_medical_signers", { role: "worker", round: currentRound }, () => getMedicalSignerSnapshot());
                        console.log(`Fetched ${medicalSignerSnapshot.signers.length} approved medical signer keys ` +
                    ...
                    try {
                        fetchedGlobalModel = await runOperation("worker.fetch_global_model", { role: "worker" }, () => getCurrentModel(activeParticipantKey.privateKey));
                    }
                    catch (error) {
            ...
            case "AGGREGATING":
                if (sameAddress(state[1], process.env.ACCOUNT_ADDRESS)) {
                    ...
                    try {
                        ...
                        if (count !== expected) {
                            console.log(`The local verified input set (${count}) does not exactly match ` +
                                `the immutable on-chain input count (${expected}). ` +
                                "Keeping AGGREGATING so timeout recovery can replace this aggregator.");
                            await sleep(5000);
                            continue;
                        }
                        if (!sameHex(aggregationPolicy.algorithmHash, FEDERATED_AVERAGING_V1_HASH)) {
                        ...
                        if (!sameHex(aggregationPolicy.validationDataHash, ZERO_BYTES32)
                            || Number(aggregationPolicy.maxLossIncreaseBps) !== 0) {
                            throw new Error("The active FedAvg baseline requires a zero validation-data hash " +
            ...
            case "UPDATING":
                if (sameAddress(state[1], process.env.ACCOUNT_ADDRESS)) {
                    ...
                    const selectionGapRecovery = await recoverSelectionGap(recoveryRound, String(state[1]), "UPDATING");
                    if (selectionGapRecovery !== null) {
                        ...
                        if (selectionGapRecovery) {
                            await stageCleaning();
                            console.log("Recovered aggregator selection from on-chain state; " +
