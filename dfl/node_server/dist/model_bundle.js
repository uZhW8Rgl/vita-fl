const normalizedIdentity = (value, label) => {
    const identity = {
        ...
        publisherPublicKeyDerHex: String(value?.publisherPublicKeyDerHex || "").toLowerCase(),
        modelRound: Number(value?.modelRound),
    };
    if (!identity.modelCid
        || !identity.sigCid
...
export const reconcileFetchedGlobalModel = (fetchedValue, currentValue) => {
    ...
    if (fetched.modelCid !== current.modelCid
        ...
        throw new Error("Fetched global model, finalized round, or publisher key is no longer the active on-chain bundle; skipping local training.");
    }
    if (typeof fetchedValue.plaintextSignaturePresent !== "boolean") {
        throw new Error("Fetched global model is missing verified plaintext-signature metadata.");
    }
    const aggregationEvidence = fetchedValue.aggregationEvidence;
    if (aggregationEvidence !== null
        && (!aggregationEvidence
    ...
    if (aggregationEvidenceHash !== null
        && !/^0x[0-9a-f]{64}$/.test(String(aggregationEvidenceHash || "").toLowerCase())) {
    ...
    if ((aggregationEvidence === null) !== (aggregationEvidenceHash === null)) {
        throw new Error("Fetched global model aggregation evidence and hash presence do not match.");
    }
    return {
        ...
        aggregationEvidenceHash: aggregationEvidenceHash === null
            ? null
