...
export function shouldStartAggregation({
    expectedModels,
    presentModels,
    elapsedMs,
    deadlineMs,
    deadlineExpired = elapsedMs >= deadlineMs,
}) {
    ...
    if (presentModels <= 0) return false;
...
export function canCloseRoundInputs(policy, presentModels, nowMs = Date.now()) {
    if (!policy.opened || policy.closed) return false;
    // Every accepted commitment must have its authenticated local model file.
    if (presentModels < policy.acceptedSubmissions) return false;
...
export async function waitForRoundInputs({
    round,
    readPolicy,
    countModels,
    pollMs = 2000,
    ...
    while (true) {
        ...
        if (!policy.opened) throw new Error(`Round ${round} submission window is not open.`);
        const now = nowMs();
...
export function shouldDeferAggregatorTimeout(policy, aggregationGraceMs, nowMs = Date.now()) {
    if (!policy.opened) return false;
    if (!policy.closed && nowMs < policy.deadline * 1000) return true;
