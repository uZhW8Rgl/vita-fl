...
async function registerWithLocalTdxMock() {
    ...
    const composeHash = normalizeHashHex(
        process.env.LOCAL_TDX_COMPOSE_HASH || '0x47d7ddfa97906d05b2b7e53ce888440a820598f120079ed887229ab0302982fa',
        32,
        'local mock compose hash',
    );
...
const stateMachine = async () => {
    if (process.env.DOCKER === "phala") {
        ...
        if (await hasCurrentDeviceRegistration(publicKey)) {
            ...
            const publicIp = process.env.PUBLIC_IP || "";
            const brokerIp = process.env.MSG_BROKER_IP || "";
            const { quoteHex, rtmr3EventDigests, composeHash, workerImageDigest } = await fetchLivePhalaQuote(
                async (identity) => getDeviceRegistrationReportData(
                    process.env.ACCOUNT_ADDRESS,
                    publicIp,
                    brokerIp,
                    publicKey,
                    identity.composeHash,
