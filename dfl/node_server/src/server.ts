...
async function validateHybridAggregationEvidence({
    aggregateResult,
    aggregationPolicy,
    ...
    if (
        ...
        && !String(aggregateResult.aggregation_evidence_path).endsWith(
            "/data/results_iid/aggregated.hybrid-r.json",
        )
    ) {
        throw new Error(
            `Hybrid-R returned an unexpected evidence path: ` +
            `${aggregateResult.aggregation_evidence_path}`,
        );
    }
    ...
    if (canonicalJson(onDisk) !== canonicalJson(evidence)) {
        throw new Error(
            "Hybrid-R response evidence does not match the evidence prepared for publication.",
        );
    }
    if (!sameHex(evidence.algorithm_hash, aggregationPolicy.algorithmHash)) {
        throw new Error(
            `Hybrid-R algorithm hash mismatch: policy=${aggregationPolicy.algorithmHash}, ` +
    ...
    if (
        ...
        || !sameHex(
            evidence.validation_data_hash,
            aggregationPolicy.validationDataHash,
        )
    ) {
        throw new Error(
            `Hybrid-R validation-data hash mismatch: policy=` +
            `${aggregationPolicy.validationDataHash}, evidence=${evidence.validation_data_hash}`,
        );
    }
    if (Number(evidence.source_round) !== sourceRound) {
        throw new Error(
            `Hybrid-R evidence source round ${evidence.source_round} does not match ${sourceRound}.`,
        );
    }
    if (Number(evidence.input_count) !== inputCount) {
        throw new Error(
            `Hybrid-R evidence input count ${evidence.input_count} does not match ${inputCount}.`,
        );
    }
    const outputModelHash = await modelFileHash(
        "./data/results_iid/aggregated.bin",
    );
    if (!sameHex(evidence.output_model_sha256, outputModelHash)) {
        throw new Error(
            `Hybrid-R evidence output-model hash mismatch: file=${outputModelHash}, ` +
            `evidence=${evidence.output_model_sha256}.`,
        );
    }
    if (
        Number(evidence.max_loss_increase_bps)
        !== Number(aggregationPolicy.maxLossIncreaseBps)
    ) {
        throw new Error(
            `Hybrid-R safety-gate mismatch: policy=${aggregationPolicy.maxLossIncreaseBps}, ` +
            `evidence=${evidence.max_loss_increase_bps}.`,
        );
    }
    if (
        !Array.isArray(evidence.candidate_scores)
    ...
    if (
        (gatePassed && evidence.output_kind !== "candidate")
        || (!gatePassed && evidence.output_kind !== "parent_fallback")
    ) {
        throw new Error(
            "Hybrid-R gate result and selected output kind are inconsistent.",
        );
    }
    ...
    for (const [field, expected] of Object.entries({
        ...
        if (String(snapshotEvidence?.[field]) !== String(expected)) {
            throw new Error(
                `Hybrid-R medical-signer snapshot mismatch for ${field}.`,
            );
        }
    }
    return evidence;
...
async function assertFetchedGlobalModelIsStillCurrent(fetchedGlobalModel) {
    ...
    if (
        ...
        throw new Error(
            "Fetched global model or publisher key is no longer the active on-chain bundle; skipping local training."
        );
    }
            ...
            case "AGGREGATING":
                if (sameAddress(state[1], process.env.ACCOUNT_ADDRESS)) {
                    ...
                    try {
                        ...
                        if (
                            (process.env.DATASET_NAME || "mnist").toLowerCase()
                            !== "chestmnist"
                        ) {
                            throw new Error(
                                "Hybrid-R aggregation requires the separately signed " +
                        ...
                        const medicalSignerSnapshot = await runOperation(
                            "aggregator.fetch_medical_signers",
                        ...
                        const aggregateResult = await runOperation("aggregator.aggregation", {
                            ...
                            expected_validation_data_hash:
                                aggregationPolicy.validationDataHash,
                            max_loss_increase_bps:
                                aggregationPolicy.maxLossIncreaseBps,
                            private_key: participantPrivateKeyRuntimePath,
                        }, { timeoutMs: 3 * 60 * 1000 }));
                        const aggregationEvidence =
                            await validateHybridAggregationEvidence({
                                aggregateResult,
                                aggregationPolicy,
                                medicalSignerSnapshot,
                        ...
                        console.log("Hybrid-R selection evidence:", {
                            selectedCandidate:
                                aggregationEvidence.selected_candidate,
                            ...
                            validationDataHash:
                                aggregationEvidence.validation_data_hash,
                        });
                        await runtimeEvent("aggregator.hybrid_r.selected", {
                            ...
                            selected_candidate:
                                String(aggregationEvidence.selected_candidate),
                            output_kind:
                                String(aggregationEvidence.output_kind),
                            gate_passed:
                                Boolean(aggregationEvidence.gate_passed),
                            parent_loss:
                                Number(aggregationEvidence.parent_loss),
                            selected_loss:
                                Number(aggregationEvidence.selected_loss),
                            max_loss_increase_bps:
                                Number(
                                    aggregationEvidence.max_loss_increase_bps,
                                ),
                        });
