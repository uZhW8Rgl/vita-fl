...
export type FrozenBootstrapRecipient = {
    address: string;
    publicKeyDerHex: string;
};

export type CommittedRunRosterBinding = {
    chainId: number;
    registryAddress: string;
    rosterDigest: string;
    bootstrapWorker: string;
    workerCount: number;
    recipientAddresses: string[];
};

type PersistedBootstrapRecipient = {
    address: string;
    public_key_der_hex: string;
};

export type RoundZeroBootstrapSnapshot = {
    version: 3;
    chain_id: number;
    registry_address: string;
    run_roster_digest: string;
    bootstrap_worker: string;
    worker_count: number;
    recipients: PersistedBootstrapRecipient[];
};
...
export const normalizeRunRosterDigest = (
    value: unknown,
    label = "committed run-roster digest",
) => {
    const normalized = String(value || "").trim().toLowerCase();
...
export const normalizeBootstrapPublicKey = (
    value: unknown,
    label = "bootstrap recipient RSA public key",
) => {
...
const normalizeRecipientAddresses = (values: unknown) => {
    if (!Array.isArray(values) || values.length === 0) {
        throw new Error("Committed run roster requires at least one recipient address.");
    }
    const addresses = values.map((value, index) => normalizeAddress(
        value,
        `committed run-roster recipient ${index}`,
    ));
...
export const createCommittedRunRosterBinding = ({
    chainId,
    registryAddress,
    rosterDigest,
    recipientAddresses,
...
const normalizeFrozenRecipients = (values: unknown): FrozenBootstrapRecipient[] => {
    if (!Array.isArray(values) || values.length === 0) {
    ...
    const recipients = values.map((value, index) => {
        ...
        return {
            address: normalizeAddress(
                entry.address,
                `round-0 bootstrap recipient ${index}`,
            ),
            publicKeyDerHex: normalizeBootstrapPublicKey(
                entry.publicKeyDerHex ?? entry.public_key_der_hex,
                `round-0 bootstrap recipient ${index} RSA public key`,
            ),
        };
    });
...
export const buildRoundZeroBootstrapSnapshot = (
    binding: CommittedRunRosterBinding,
    recipientEntries: FrozenBootstrapRecipient[],
): RoundZeroBootstrapSnapshot => {
    ...
    if (JSON.stringify(addresses) !== JSON.stringify(binding.recipientAddresses)) {
        throw new Error(
            "Frozen round-0 recipient entries do not match the committed run roster.",
        );
    }
    return {
        version: 3,
...
export const parseRoundZeroBootstrapSnapshot = (
    value: unknown,
): RoundZeroBootstrapSnapshot => {
    if (!value || typeof value !== "object" || Array.isArray(value)) {
    ...
    if (snapshot.version !== 3) {
        throw new Error(
            "Round-0 bootstrap snapshot is not version 3 bound to the committed run roster.",
        );
    }
    ...
    const bootstrapWorker = normalizeAddress(
        snapshot.bootstrap_worker,
        "round-0 bootstrap snapshot worker",
    );
    if (recipients[0].address !== bootstrapWorker) {
        throw new Error(
            "Round-0 bootstrap snapshot worker is not the first recipient.",
        );
    }
    return {
        version: 3,
        chain_id: chainId,
        registry_address: normalizeAddress(
            snapshot.registry_address,
            "round-0 bootstrap snapshot registry",
        ),
        run_roster_digest: normalizeRunRosterDigest(
            snapshot.run_roster_digest,
            "round-0 bootstrap snapshot run-roster digest",
        ),
...
export const frozenRecipientsForCommittedRoster = (
    snapshotValue: unknown,
    binding: CommittedRunRosterBinding,
): FrozenBootstrapRecipient[] => {
    ...
    for (const [field, expected] of Object.entries(expectedContext)) {
        if (snapshot[field as keyof RoundZeroBootstrapSnapshot] !== expected) {
            throw new Error(
                `Round-0 bootstrap snapshot does not match the committed ${field}.`,
            );
        }
    }
    ...
    if (
        JSON.stringify(recipients.map(({ address }) => address))
        !== JSON.stringify(binding.recipientAddresses)
    ) {
        throw new Error(
            "Round-0 bootstrap snapshot recipients do not match the committed run roster.",
        );
    }
    return recipients;
};

export const requireFrozenRecipientKeysMatchRegistry = (
    frozenValues: FrozenBootstrapRecipient[],
    registeredValues: FrozenBootstrapRecipient[],
): FrozenBootstrapRecipient[] => {
    ...
    if (frozen.length !== registered.length) {
        throw new Error(
            "Frozen round-0 recipient keys do not match the immutable DeviceRegistry keys.",
        );
    }
    for (let index = 0; index < frozen.length; index++) {
        if (
            frozen[index].address !== registered[index].address
            || frozen[index].publicKeyDerHex !== registered[index].publicKeyDerHex
        ) {
            throw new Error(
                `Frozen round-0 recipient ${index} key does not match the `
