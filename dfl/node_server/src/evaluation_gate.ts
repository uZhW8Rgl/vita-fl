/** Cooperative evaluation-only fault injection. No gate URL means no extra I/O. */
...
export async function evaluationUploadDecision(
    context: GateContext,
    options: { env?: NodeJS.ProcessEnv; request?: typeof fetch } = {},
): Promise<{ allow: boolean; run_id?: string }> {
    const env = options.env || process.env;
    const configured = env.DFL_EVALUATION_GATE_URL || '';
    if (!configured) return { allow: true };
    const origin = new URL(configured);
    if (origin.protocol !== 'https:' || origin.username || origin.password || origin.search || origin.hash ||
        !['', '/'].includes(origin.pathname)) throw new Error('Invalid evaluation gate HTTPS origin');
    ...
    if (!/^[a-zA-Z0-9_-]{32,128}$/.test(runId)) throw new Error('Missing pinned evaluation gate generation');
    ...
    if (context.round > 1) return {allow: true, run_id: runId};
    ...
    if (response.status !== 200) throw new Error('Evaluation gate decision unavailable');
    const raw = await response.text();
    if (raw.length > 4096) throw new Error('Oversized evaluation gate decision');
