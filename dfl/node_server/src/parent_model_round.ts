export type AggregatorParentRoundContext = {
    modelRound: number;
    currentRound: number;
};

const requirePositiveRound = (value: number, label: string): number => {
 ...
 * never usable.
 */
export const skippedAggregatorAttemptRounds = ({
    modelRound,
    currentRound,
}: AggregatorParentRoundContext): number[] => {
    const finalizedModelRound = requirePositiveRound(
        modelRound,
        "Active global-model round",
    );
    const aggregationRound = requirePositiveRound(
        currentRound,
        "Current aggregation round",
    );
    if (finalizedModelRound > aggregationRound) {
        throw new Error(
            `Active global-model round ${finalizedModelRound} is from the future `
            + `relative to aggregation round ${aggregationRound}.`,
        );
    }

    return Array.from(
        { length: aggregationRound - finalizedModelRound },
        (_unused, index) => finalizedModelRound + index,
    );
};

export const requireAbortedAggregatorAttemptGap = ({
    modelRound,
    currentRound,
    abortedAttemptRounds,
}: AggregatorParentRoundContext & {
    abortedAttemptRounds: Iterable<number>;
}): number[] => {
    ...
    for (const round of abortedAttemptRounds) {
        if (!Number.isSafeInteger(round) || round <= 0) {
            throw new Error(
                `Aborted contract-attempt round must be a positive safe integer; got ${round}.`,
            );
        }
        aborted.add(round);
    }
    ...
    if (unresolved.length > 0) {
        throw new Error(
            `Active global-model round ${modelRound} predates aggregation round `
