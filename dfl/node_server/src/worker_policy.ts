...
const extractDockerCompose = (canonicalAppCompose: Buffer): Buffer => {
    ...
    for (let index = offsets[0]; index < canonicalAppCompose.length; index += 1) {
        ...
        index += 1;
        if (index >= canonicalAppCompose.length) throw new Error("invalid JSON escape");
        const escaped = canonicalAppCompose[index];
        if (escaped === 0x22 || escaped === 0x5c || escaped === 0x2f) output.push(escaped);
        else if (escaped === 0x62) output.push(0x08);
        else if (escaped === 0x66) output.push(0x0c);
        else if (escaped === 0x6e) output.push(0x0a);
        else if (escaped === 0x72) output.push(0x0d);
        else if (escaped === 0x74) output.push(0x09);
        else throw new Error("unsupported JSON escape");
    }
    if (!terminated) throw new Error("unterminated docker compose field");
