...
def _equal_weight_federated_average(models: list[FederatedCNN]) -> FederatedCNN:
    if not models:
        raise ValueError("Federated averaging requires at least one model")
    output_model = FederatedCNN().double()
    output_state = {}
    for key in output_model.state_dict().keys():
        averaged = torch.stack([model.state_dict()[key] for model in models]).mean(dim=0)
        if not torch.isfinite(averaged).all():
            raise ValueError(f"Federated average contains a non-finite parameter in {key}")
        output_state[key] = averaged
...
def aggregate(
    ...
    *,
    ...
    models = [read_model_bin(path) for _, path in canonical_inputs]
    output_model = _equal_weight_federated_average(models)
